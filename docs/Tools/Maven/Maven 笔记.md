...
只有 一个文件（maven 3.3.9 中） `plexus-classworlds-2.5.2.jar` 类加载器。一般用不到。
...
> maven clean package (pom 文件中没有定义打包类型，默认打包为 jar)
>
> 与编译 类似。

### 2.3 使用 archetype 生成项目骨架
...
ecilpse 默认 运行在 jre 上，m2 需要用 jdk.
...
当前 项目隶属于实际项目。maven 项目与实际项目 不是一对一关系。例子：SpringFarmework，子项目 有 spring-core ...
...
项目 1 依赖了 项目 2，项目 2 依赖 3，但是 项目 1，选择传递依赖 项目 3.而是 自己选择版本 1.1

## 4. 仓库

本地 与 远程。 远程 ： Maven 中央仓库，私服， java.net maven 库 Jboss Maven 库。
...
Profile 能让你为一个特殊的环境自定义一个特殊的构建；profile 使得不同环境间构建 的可移植性成为可能
